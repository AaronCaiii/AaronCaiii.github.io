...
这里显示用户`cmeeks`有权限去修改 `pythonapp.service` 里面的内容,查看一下这个配置文件的内容.
...
在这里可以看到`ExecStart`在运行命令, 并且当前用户有权限去修改此配置, 所以修改成反弹shell的命令.
...
然后使用`sudo /sbin/reboot` 来重启服务器, 等待一会就能接收到反弹shell
